jQuery(document).ready(function($) {
    ...
    function insert_rule(country_code, from, to, url_from, url_to, http_code) {
        var countries = {
            "": "",
            "AF": "Afghanistan",
            "AL": "Albania",
        ...
        var $country_list = $('<select name="country_code[]" data-placeholder="Choose Country..." class="chosen country-code">').on('change', function() {
            if (is_duplicated_rule(this)) {
                alert('This rule will conflicts with others.');
                $(this).val('');
        ...
        $.each(countries, function(iso_code, country_name) {
            if (country_code.substr(0, 1) == '-') {
                exclude = true;
                country_code = country_code.substr(1);
            }

            $country_list.append('<option value="' + iso_code + '"' + ((iso_code == country_code) ? ' selected' : '') + '>' + country_name + '</option>');
        });
        ...
        $exclude_checkbox.on('change', function() {
            ...
            if (is_duplicated_rule($(this).parent().parent())) {
                alert('This rule will conflicts with others.');
        ...
        var $from_list = $('<select name="from[]" data-placeholder="Choose Page..." class="chosen redirect-from">')
            ...
            .on('change', function() {
                ...
                ($(this).val() == 'url') ? $(this).parent().find('.url-container').slideDown(): $(this).parent().find('.url-container').slideUp();

				if ($(this).val() == 'domain') {
					$(this).parent().find('.domain-container').slideDown();
					...
					$(this).parent().find('.domain-container').slideUp();
				}
        ...
        var $rule = $('<tr>')
            .append($('<td>').append($country_list).append($('<p />').append($('<label />').append($exclude_checkbox).append($exclude_input).append(' Redirect all countries <strong>except</strong> country listed above.'))))
    ...
    function is_duplicated_rule(obj) {
        var $row = $(obj).parent().parent();
        var country_code = $row.find('.country-code').val();
        var from = $row.find('.redirect-from').val();
        var exclude = $row.find('.exclude-checkbox').is(':checked');
        var is_duplicated = false;

        $('#rules tr').not($row).each(function(i, ele) {
            if ($(ele).find('.country-code').val() != country_code) {
                if (exclude || $(ele).find('.exclude').is(':checked')) {
                    if ($(ele).find('.redirect-from').val() == from) {
                        is_duplicated = true;
                        return;
                    }
                }
            ...
            if ($(ele).find('.country-code').val() == country_code) {
                if ($(ele).find('.redirect-from').val() == 'any' || from == 'any') {
                    is_duplicated = true;
                    return;
                }

...
function wonderplugin_gridgallery_dirtoarray($dir, $recursive) {
	...
	foreach ($cdir as $key => $value)
	{
		if (!in_array($value,array(".","..")))
		{
			if (is_dir($dir . DIRECTORY_SEPARATOR . $value))
			{
				if ($recursive)
				{
					$result[$value] = wonderplugin_gridgallery_dirtoarray($dir . DIRECTORY_SEPARATOR . $value, $recursive);
				}
				else
				{
					$result[] = $value;
...
function wonderplugin_gridgallery_tags_allow( $allowedposttags ) {
	...
	$allowedposttags = apply_filters( 'wonderplugin_gridgallery_custom_tags_allow', $allowedposttags );
	
	return $allowedposttags;
}

function wonderplugin_gridgallery_css_allow($allowed_attr) {
	...
	$allowed_attr = apply_filters( 'wonderplugin_gridgallery_custom_css_allow', $allowed_attr );

...
class Genesis_Palette_Pro
{
	...
	public function deactivate( $genesis_version = '2.2.0', $wp_version = '4.0' ) {
		...
		// Show the message.
		wp_die( sprintf( __( 'Sorry, you cannot run Design Palette Pro without WordPress %s and <a href="%s">Genesis %s</a> or greater.', 'gppro' ), $wp_version, 'https://my.studiopress.com/themes/genesis/', $genesis_version ) );
	}
	 ...
	 *
	 * @return string 'yes' if viewable, 'no' otherwise.
	 */
	public static function file_access_check( $file = '' ) {
		...
		if ( false === $access = get_transient( 'gppro_check_file_access' ) ) {
			...
			if ( is_wp_error( $fetch ) ) {
				...
				// Set the transient.
				set_transient( 'gppro_check_file_access', 'no', DAY_IN_SECONDS );
			...
			if ( ! is_wp_error( $fetch ) ) {
				...
				if ( is_wp_error( $code ) || empty( $code ) || absint( $code ) !== 200 ) {
			...
			set_transient( 'gppro_check_file_access', 'yes', DAY_IN_SECONDS );

			// Return the access string.
			return 'yes';
		}

		// Return whatever we had stored.
		return $access;
	}

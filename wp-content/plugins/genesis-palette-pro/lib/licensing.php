...
class GP_Pro_Licensing {
	...
	public function license_scripts_styles() {
		...
		// Ensure we are on settings page under the "Tools" menu first.
		...
		// Load our CSS file.
		wp_enqueue_style( 'gppro-licensing', plugins_url( '/css/licensing' . $vars['style'], __FILE__ ), array(), $vars['vers'], 'all' );

		// Load our JS files.
		wp_enqueue_script( 'hideShowPassword', plugins_url( 'js/ext/hideShowPassword' . $vars['script'], __FILE__ ), array( 'jquery' ), '2.0.11', true );
		wp_enqueue_script( 'gppro-licensing', plugins_url( 'js/licensing' . $vars['script'], __FILE__ ), array( 'jquery' ), $vars['vers'], true );
	...
	public function check_license() {
		...
		if ( false === $status = get_transient( 'gppro_core_license_verify' ) ) {
			...
			// Delete our existing license metadata.
	...
	public static function license_field_layout() {
		...
		// Build my reset link.
		$reset   = add_query_arg( array( 'gppro-purge' => 1, 'tools-page' => 1 ), admin_url() );
	...
	public static function license_meta_display( $space = '&nbsp;' ) {
		...
		if ( empty( $data['pastdue'] ) ) {
			...
			if ( empty( $data['unlimited'] ) ) {
				...
				// Build the meta array.
				if ( $count === $limit ) {
					$meta[] = sprintf( __( 'You have used your total of %1$d activations.', 'gppro' ), absint( $data['limit'] ) );
				} else {
	...
	public function manual_activation() {
		...
		if ( empty( $check['status'] ) ) {
			...
			// Set my redirect link with the error code.
			$link   = add_query_arg( array( 'processed' => 'failure', 'errcode' => $error ), $link );

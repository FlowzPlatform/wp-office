...
class GP_Pro_Admin
{
	...
	public function admin_styles() {
		...
		wp_enqueue_style( 'gppro-admin', plugins_url( 'css/admin' . $vars['style'], __FILE__ ), array(), $vars['vers'], 'all' );
	...
	public function admin_scripts() {
		...
		wp_enqueue_script( 'screenfull', plugins_url( 'js/ext/screenfull' . $vars['script'], __FILE__ ), array( 'jquery' ), '2.0.0', true );

		// Load and localize the color picker.
		wp_enqueue_style( 'wp-color-picker' );
		wp_enqueue_script( 'wp-color-picker-alpha', plugins_url( 'js/ext/wp-color-picker-alpha' . $vars['script'], __FILE__ ), array( 'wp-color-picker' ), '2.0', true );

		// Only load the preview JS if we have a preview.
		if ( false === apply_filters( 'gppro_disable_preview_pane', false ) ) {
			wp_enqueue_script( 'gppro-preview', plugins_url( 'js/preview' . $vars['script'], __FILE__ ), array( 'jquery' ), $vars['vers'], true );
		}

		// Load and localize our admin file.
		wp_enqueue_script( 'gppro-admin', plugins_url( 'js/admin' . $vars['script'], __FILE__ ), array( 'wp-color-picker', 'jquery-ui-core', 'jquery-ui-slider', 'jquery-ui-tooltip', 'jquery' ), $vars['vers'], true );
	 ...
	 *
	 * Denies all files from the directory except .css files.
	 * Also removes index visibility.
	 *
	...
	public static function get_htaccess() {
		...
		return apply_filters( 'gppro_htaccess_content', $data );
	}

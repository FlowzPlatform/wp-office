...
class GP_Pro_Notices {
	...
	public function init() {
		...
		if ( empty( $_GET['page'] ) || ! in_array( sanitize_key( $_GET['page'] ), array( 'genesis-palette-pro', 'dpp-license-tools' ) ) ) { // Input var okay.
			return;
		}
	...
	public function php_version_notice() {
		...
		if ( false !== $ignore = GP_Pro_Helper::get_single_option( 'gppro-warning-phpvers', '', false ) ) {
			return;
		}
	...
	public function viewable_notice() {
		...
		if ( 'no' !== $view = Genesis_Palette_Pro::file_access_check( $file ) ) {
			return;
		}
	...
	public function create_debug_notice() {
		...
		// Show cache message.
		if ( ! empty( $_GET['cache'] ) ) { // Input var okay.

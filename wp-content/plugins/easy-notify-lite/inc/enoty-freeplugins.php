...
function easynotify_rec_init() {
    $easynotify_featured_page = add_submenu_page('edit.php?post_type=easynotify', 'Free Install Plugins', __('Free Install Plugins', 'easy-notify-lite'), 'edit_posts', 'easynotify_free_plugins', 'easynotify_free_plugin_page');
}
add_action( 'admin_menu', 'easynotify_rec_init' );


function easynotify_free_plugin_page() {
	
	add_thickbox(); // @since 1.1.13
...
<div class="wrap">
	...
	.easynotify-plugin-action-buttons {
		text-align:right;
		}
	.easynotify-plugin-action-buttons li {
		display: inline-block;
		margin-left: 1em;
		}
		
	.easynotify-button-con {
		padding-right: 15px;
		}
		
.easynotify-button-install {
...
.easynotify-button-update:focus {
    box-shadow: 0px 0px 0px 1px #DA3232, 0px 0px 2px 1px rgba(255, 140, 140, 0.8) !important;
}
	
.easynotify-button-update:hover {
    color: #FFF !important;
	background: none repeat scroll 0% 0% #DA3232 !important;
}

.most_popular {top: -5px;position: absolute;width: 115px;height: 85px;background: transparent url("<?php echo ENOTIFY_URL;?>/inc/images/most_popular.png") no-repeat scroll left top;right: -5px;}
...
.easynotify_page_easynotify_free_plugins .plugin-card {
    border: 1px solid #d7d7d7 !important;
}

.easynotify_page_easynotify_free_plugins .plugin-card-bottom {
				...
				switch ( $status['status'] ) {
					case 'install':
						if ( $status['url'] ) {
							...
							$action_links[] = '<a class="install-now button-secondary easynotify-button-install" href="' . $status['url'] . '" aria-label="' . esc_attr( sprintf( __( 'Install %s now' ), $name ) ) . '">' . __( 'Install Now' ) . '</a>';
						}
					...
					case 'update_available':
						if ( $status['url'] ) {
							/* translators: 1: Plugin name and version */
							$action_links[] = '<a class="button easynotify-button-update" href="' . $status['url'] . '" aria-label="' . esc_attr( sprintf( __( 'Update %s now' ), $name ) ) . '">' . __( 'Update Now' ) . '</a>';
						}
		...
		<div id="<?php echo $plugin["slug"]; ?>" class="plugin-card drop-shadow lifted">
            ...
            <?php if ( isset( $plugin["slug"] ) && $plugin["slug"] == 'easy-media-gallery' ) {echo '<div class="most_popular"></div>';} ?>
				...
				<div class="desc column-description" style="margin-right: 20px !important;">
					...
					<div class="easynotify-button-con">
					<?php
						if ( $action_links ) {
							echo '<ul class="easynotify-plugin-action-buttons">';
...
jQuery(document).ready(function($) {
	...
	var theListspp = ['easy-media-gallery', 'contact-form-lite', 'feed-instagram-lite', 'image-slider-widget', 'gallery-lightbox-slider', 'image-carousel','icon', 'easy-notify-lite'];
	
	$('#the-list').enlReOrder(theListspp);

...
if ( ! class_exists( 'TCB_Editor_Ajax' ) ) {
	...
	class TCB_Editor_Ajax {
		...
		/**
		 * Return widget options form
		 * @return array
		 */
		public function action_widget_form() {
			global $wp_widget_factory;

			$widget_type = $this->param( 'widget' );
			$form_data   = $this->param( 'data' );

			$form_data = wp_unslash( str_replace( '__CONFIG_thrive_widget__', '', $form_data ) );
			$form_data = json_decode( $form_data, true );
			$form_data = is_array( $form_data ) && ! empty( $form_data ) ? $form_data : array();

			$content = '';

			foreach ( $wp_widget_factory->widgets as $widget ) {
				if ( $widget->option_name === $widget_type ) {
					$content = tcb_template( 'widget-form.php', compact( 'widget', 'form_data' ), true );
				}
			}

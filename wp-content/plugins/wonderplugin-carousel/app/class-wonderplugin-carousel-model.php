	...
	function create_post_code($slides, $data, $id) {
		...
		foreach($slides as $slide)
		{
			...
			if ( isset($slide->categories) )
			{
				...
				$skin_template = preg_replace('/__CATEGORYNAME\[(.*)\]__/', implode('$1', array_column($slide->categories, 'name')), $skin_template);
			...
			if ( isset($slide->tags) )
			{
				...
				$skin_template = preg_replace('/__TAGNAME\[(.*)\]__/', implode('$1', array_column($slide->tags, 'name')), $skin_template);

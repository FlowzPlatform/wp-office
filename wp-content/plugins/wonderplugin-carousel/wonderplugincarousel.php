...
define('WONDERPLUGIN_CAROUSEL_VERSION', '13.1C');
...
define('WONDERPLUGIN_CAROUSEL_PLUGIN_VERSION', '13.1');
...
class WonderPlugin_Carousel_Plugin {
	...
	public function init() {
		...
		if ( is_admin() )
		{
			...
			add_action( 'wp_ajax_wonderplugin_carousel_list_folder', array($this, 'wp_ajax_list_folder') );
	...
	function register_menu()
	{
		...
		$menu = add_submenu_page(
				'wonderplugin_carousel_overview',
				__('Register', 'wonderplugin_carousel'),
				__('Register', 'wonderplugin_carousel'),
				'manage_options',
				'wonderplugin_carousel_register',
				array($this, 'register' ) );
		add_action( 'admin_print_styles-' . $menu, array($this, 'enqueue_admin_script') );
	...
	function shortcode_handler($atts, $content = null) {
		...
		return $this->generate_lightbox_options() . "\r\n" . $this->wonderplugin_carousel_controller->generate_body_code( (isset($atts['id']) ? $atts['id'] : null), (isset($atts['name']) ? $atts['name'] : null), false, $inline_content, $attributes);
	}
	...
	function wp_ajax_list_folder() {
		...
		echo json_encode(wonderplugin_carousel_dirtoarray($folder, false));
		wp_die();
	}
...
define('WONDERPLUGIN_CAROUSEL_VERSION_TYPE', 'C');
